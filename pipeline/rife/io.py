"""Frame image I/O for Step 3 (decoupled for testability).

Production uses :class:`Cv2FrameIO` (BGR files <-> RGB arrays). Unit tests
inject an in-memory fake, so the suite runs without cv2/numpy/torch.
"""
...
class Cv2FrameIO(FrameIO):
    ...
    def _cv2(self):  # noqa: ANN202 - cv2 module
        try:
            ...
            raise RuntimeError(
                "Step 3 needs opencv-python for frame I/O. Install it with "
